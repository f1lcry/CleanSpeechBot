...
Telegram-бот для локальной обработки голосовых сообщений и выдачи отформатированного текста. Сейчас бот полностью функционирует в личных и групповых чатах Telegram: получает голосовые сообщения, прогоняет их через локальный конвейер (FFmpeg → Whisper → Llama 3.1 через Ollama) и возвращает компактное саммари пользователю. README служит единым источником правды о проекте: архитектура, пайплайн, требования, статус реализации и планы.
...
- **Handlers** — маршруты `text_router`, `voice_router` и `chat_member_router` в `bot/handlers/` принимают команды и голосовые вложения, приветствуют группы и делегируют работу пайплайну только после нажатия пользователем кнопки "Сделать саммари".
- **Служебные утилиты**
...
1. **Приём голосового сообщения.** Aiogram получает сообщение, сохраняет метаданные запроса и отвечает сервисной репликой с кнопкой «Сделать саммари» под оригинальным голосовым.
2. **Запуск по кнопке.** Как только пользователь нажимает кнопку, бот скачивает файл и отправляет задачу в `TaskQueueManager`, чтобы ограничивать параллелизм и нагрузку на CPU/GPU.
