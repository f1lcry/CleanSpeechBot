...
- **Добавление в группу.** Как только бот становится участником (`my_chat_member` → `member/administrator`), он отправляет приветствие с подсказкой выдать права администратора — без них Telegram не присылает обычные сообщения. После повышения до администратора бот сообщает, что готов к работе, и кратко напоминает сценарий с кнопкой «Сделать саммари».
...
Приоритеты сформированы исходя из текущего состояния (бот стабильно работает в личных чатах и группах) и дальнейших планов владельца проекта:

1. **Отдельный лог-бот или лог-чат.**
   - Вести технические логи в отдельном приватном канале/боте для продакшен-развёртывания.
2. **База данных пользователей и чатов.**
   - Отслеживать уникальных пользователей, количество запросов и формат взаимодействия для аналитики и лимитов.
3. **Расширенная аналитика и метрики.**
   - Счётчики задач, длительность этапов, прогресс-статусы для пользователя, уведомления об ошибках.
4. **Умный UX в группах.**
   - Публичные статусы обработки, продление TTL временных файлов, разграничение ролей (админы vs пользователи), интерактивный inline-keyboard.
...
- Бот полностью работает в личных и групповых чатах Telegram: есть приветствие при добавлении в чат, проверка статуса администратора, inline-кнопка «Сделать саммари», TTL временных файлов и защита от повторных нажатий.
- CLI-инструменты позволяют отдельно тестировать каждый модуль пайплайна.
- В README задокументирован end-to-end сценарий, включая особенности группового UX и ограничения.
...
| `v0.2.0` | Интеграция в групповые чаты: приветствия при добавлении, валидация прав администратора, inline-кнопка «Сделать саммари», очистка временных файлов и защита от повторного запуска пайплайна. |
